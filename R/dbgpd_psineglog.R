dbgpd_psineglog <-
function(x,y,mar1=c(0,1,0.1),mar2=c(0,1,0.1),dep=1,a=1,b=1,asy=0,...)
{

A                 = function(x,alpha) 1 - (x^(-alpha) + (1 - x)^(-alpha))^(-1/alpha)
d1A               = function(x,alpha) (x^(-alpha)+(1-x)^(-alpha))^(-1/alpha)*(-x^(-alpha)*
alpha/x+(1-x)^(-alpha)*alpha/(1-x))/(alpha*(x^(-alpha)+(1-x)^(-alpha)))
d2A               = function(x,alpha) -(x^(-alpha)+(1-x)^(-alpha))^(-1/alpha)*(-x^(-alpha)*
alpha/x+(1-x)^(-alpha)*alpha/(1-x))^2/(alpha^2*(x^(-alpha)+(1-x)^(-alpha))^2)+
(x^(-alpha)+(1-x)^(-alpha))^(-1/alpha)*(x^(-alpha)*alpha^2/x^2+x^(-alpha)*
alpha/x^2+(1-x)^(-alpha)*alpha^2/(1-x)^2+(1-x)^(-alpha)*alpha/(1-x)^2)/
(alpha*(x^(-alpha)+(1-x)^(-alpha)))-(x^(-alpha)+(1-x)^(-alpha))^(-1/alpha)*
(-x^(-alpha)*alpha/x+(1-x)^(-alpha)*alpha/(1-x))^2/(alpha*(x^(-alpha)+(1-x)^(-alpha))^2)
...
mu                = function(x,y,alpha,c,a) (1/x+1/y)*Afi(x/(x+y),alpha,c,a)
                  ...
                  param         = as.numeric(c(mar1,mar2,dep,asy))
...
if(sigx<0 | sigy<0 | alpha<0.7 ) error=TRUE
if(!error){
